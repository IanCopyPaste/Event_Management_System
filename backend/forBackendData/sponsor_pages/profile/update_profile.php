...
if (!empty($new_password)) {
    $stmt = $conn->prepare("SELECT password FROM sponsorships WHERE sponsor_id = ?");
...
if (isset($_FILES['new_logo']) && $_FILES['new_logo']['error'] === UPLOAD_ERR_OK) {
    $upload_dir = '../../../../image_data/sponsor_logo/'; 
...
if (count($update_fields) > 0) {
    $sql = "UPDATE sponsorships SET " . implode(", ", $update_fields) . " WHERE sponsor_id = ?";
    $types .= "i";
    $params[] = $sponsor_id;
