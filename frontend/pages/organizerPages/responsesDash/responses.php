...
<style>
    ...
    /* no glow pseudo-element needed, but keep it hidden */
    .event-card::before {
        content: "";
        position: absolute;
        top: -120px;
    ...
    .registration-footer span:first-child {
        color: #a8a29e;
        font-size: 0.875rem;
    }

    .registration-footer span:last-child {
        color: #1c1917;
        font-weight: 600;
        font-size: 0.95rem;
    }
        ...
        <?php while ($event = $events->fetch_assoc()): ?>
            ...
            <div
                ...
                onclick="
                        openModal(
                            <?= $event['event_id'] ?>,
                            '<?= htmlspecialchars(addslashes($event['event_name'])) ?>'
                        )
                    ">
    ...
    async function openModal(eventID, eventName) {
        ...
        try {
            ...
            const response = await fetch(
                `backend/forBackendData/orgNresponses/fetch_respondents.php?event_id=${eventID}`
            );
    ...
    function renderModalData(data) {
        ...
        document.getElementById(
            "modalEventStats"
                ...
                ${data.stats.total_registrants}
                registered •
                ${data.stats.remaining_slots}
                remaining slots
            `;
        ...
        document.getElementById(
            "statsGrid"
                ...
                <div class="stat-card">
                    ...
                    <p>${data.stats.total_registrants}</p>
                </div>

                <div class="stat-card">
                    <h3>Remaining Slots</h3>
                    <p>${data.stats.remaining_slots}</p>
                </div>

                <div class="stat-card">
                    <h3>Capacity Filled</h3>
                    <p>${data.stats.percentage}%</p>
                </div>
